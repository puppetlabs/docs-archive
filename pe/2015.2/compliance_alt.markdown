 ...
 * To set an individual resource as no-op, set [the `noop` metaparameter](/references/4.2.latest/metaparameter.html#noop) to `true`.
 ...
 * To do an entire Puppet run in no-op, set [the `noop` setting](/references/4.2.latest/configuration.html#no-op) to `true`. This can be done in the `[agent]` block of puppet.conf, or as a `--noop` command-line flag. If you are running Puppet agent in the default daemon mode, you would set no-op in puppet.conf.
