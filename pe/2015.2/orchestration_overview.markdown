---
layout: default
title: "PE 2015.2 » Orchestration » Overview"
...
Puppet Enterprise can invoke orchestration actions [on the command line](./orchestration_invoke_cli.html)
...
- The `mco` command (available to the `peadmin` user account on the Puppet master server) can issue authorized orchestration commands to any number of nodes.
...
The orchestration engine in Puppet Enterprise uses the same security model as the recommended "standard MCollective deployment." [See the "security model" section on the MCollective standard deployment page](/mcollective/deploy/standard.html#security-model) for a more detailed rundown of these security measures.
