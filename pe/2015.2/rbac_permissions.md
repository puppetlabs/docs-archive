---
layout: default
title: "PE 2015.2 » Console » RBAC Permissions"
...
| Users              | Revoke                                  | The ability to revoke/disable a user. This means the user will no longer be able to authenticate and use the console, node classifier, or RBAC. This may be granted per user. |
...
When granting create/import permissions, it's important to keep in mind that users, user groups, and roles are not deletable via the console, only by using the API. For this reason, we recommend that you restrict how many users have these permissions.