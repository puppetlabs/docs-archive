...
2. **On the PuppetDB**, shut down all PE-related services with the following commands:
...
3. **On the PuppetDB**, delete the Puppet agent's SSL cert and security credentials.
...
6. **On the PuppetDB**, generate security credentials and request a new certificate from the CA Puppet master. These certs will end up in `/etc/puppetlabs/puppet/ssl`.
...
7. **On the PuppetDB**, delete puppetDB’s SSL cert and security credentials.

        rm -rf /etc/puppetlabs/puppetdb/ssl/*

8. **On the PuppetDB**, copy the Puppet agent's certs and security credentials to the PuppetDB SSL directory.
...
9. **On the PuppetDB**, clear the certs and security credentials from the PostgreSQL certs directory.

         rm -rf /opt/puppetlabs/server/data/postgresql/9.4/data/certs/*

10. **On the PuppetDB**, copy the certs and security credentials to the PostgreSQL certs directory.
...
11. **On the PuppetDB**, restart all PE-related services with the following commands:
