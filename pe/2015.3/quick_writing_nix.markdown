...
1. **In the console**, click __Add classes__, choose the `pe_quickstart_app` class from the list, and then click __Add selected classes__ to make it available, just as in the [previous example](./quick_start.html#using-modules-in-the-console). You might need to wait a moment or two for the class to show up in the list.
2. Navigate to the node view page for your agent, and click __Edit__ to add the `pe_quickstart_app` class  and to remove the `apache` class you previously added.

   >**Note**: Since the `pe_quickstart_app` includes the `apache` class, you need to remove the first `apache` class you added, as Puppet will only allow you to declare a class once.

3. From the command line on your agent, run `puppet agent -t`.

     When the Puppet run is complete, you will see in the node's log that a vhost for the app was created and the Apache service (httpd) started.

4. Use a browser to navigate to port 80 of the IP address for your node: `http://<yournodeip>:80`.

   >**Tip**: Be sure to use `http`, not `https`.
...
1. **On the agent**, open `/var/www/pe_quickstart_app/index.php`, and change the content; change it to something like, "THIS APP IS MANAGED BY PUPPET!"
2. Refresh your browser, and notice that the PHP info page has been replaced with your new message.
3. After Puppet runs, refresh your browser, and notice that Puppet has reset your web app to display the PHP info page. You can also see that the content of `/var/www/pe_quickstart_app/index.php` has been reset to what was specified in your manifest.
