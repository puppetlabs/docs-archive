...
To create an empty directory in the `repo_store_root` which you can manually fill later:
...
After a `no_content` repository is created, you can log into your Razor server and fill the directory `$repo_store_root/$repo_name` with the actual content for the repository, for example by loopback mounting the install media and copying it into the repository's directory.
