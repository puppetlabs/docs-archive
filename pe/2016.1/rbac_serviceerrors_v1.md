---
layout: default
title: "RBAC service errors: API v1"
canonical: "/pe/latest/rbac_serviceerrors_v1.html"
---
...
When the client specifies an `accept` header in the request with type `application/json`, the RBAC service returns errors in a standard format. Each such response is an object containing the following keys:
...
Occurs when an unauthenticated user attempts to access a route that requires authentication. 
