...
1. In the console, click __Nodes__ > __Classification__, and then click **Add group**.
2. In the __Group name__ field, name your group, for example *firewall_example*, and then click **Add**.
3. Click the link to **add membership rules, classes and variables**.
4. On the __Rules__ tab, in the __Node name__ field, enter the name of the PE-managed node you want to add to this group, and then click **Pin node**.
...
1. In the console, click __Nodes__ > __Classification__, and select the __firewall_example__ group.

2. Click the __Classes__ tab.

3. On the **Classes** tab, in the __Class name__ field, select `my_firewall`.

   **Tip**: You only need to add the main `my_firewall` class. It contains the other classes from the module.

4. Click __Add class__, and then commit changes.

   The `my_firewall` class now appears in the list of classes for the __firewall_example__ group.

5. When Puppet runs, it configures the nodes using the newly-assigned classes. Wait one or two minutes.
