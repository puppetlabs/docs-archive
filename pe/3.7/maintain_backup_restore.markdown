...
4. Run `chown -R pe-puppet:pe-puppet /etc/puppetlabs/puppet/ssl/`.
5. Run `chown -R puppet-dashboard /opt/puppet/share/puppet-dashboard/certs/`.
6. Restore modules, manifests, hieradata, etc, if necessary.
   These are typically located in the `/etc/puppetlabs/` directory, but you may have configured them in another location.
7. Run `/opt/puppet/sbin/puppetdb-ssl-setup -f`. This script generates SSL certificates and configuration based on the agent cert on your PuppetDB node.
