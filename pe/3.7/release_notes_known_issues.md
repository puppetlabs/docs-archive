...
1. Add  `-Djava.io.tmpdir=/var/tmp` to `JAVA_ARGS` in `/etc/sysconfig/pe-puppetserver` similar to the following: `JAVA_ARGS="-Xms2048m -Xmx2048m -XX:MaxPermSize=256m -Djava.io.tmpdir=/var/tmp"`.

2. Restart `pe-puppetserver` by running:

    `puppet resource service pe-puppetserver ensure=stopped`  
    `puppet resource service pe-puppetserver ensure=running`


3. Add `{"Djava.io.tmpdir=/var/tmp":""}` as the value for the `$java_args` parameter of the `puppet_enterprise::profile::master` class using the PE console.


4. **On the Puppet master**, run `puppet agent -t`.
...
When installing PE 3.7.2, if you attempt to install an existing PostgreSQL instance on the same node as the one assigned the PuppetDB component, the install will fail with the following error: 
...
For example, in an **Agent-specified** node group with the **Environment Override** option selected, if you add a rule that matches nodes with the "test" fact set to true and then you run Puppet, the agent receives the environment that would have been assigned without the "test" fact set to true on the first Puppet run, and then on the second Puppet run it receives the agent-specified environment. 
...
This issue has been fixed in PE 3.8.
