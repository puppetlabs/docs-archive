...
This rake task exports nodes in CSV format. The CSV data is written directly to STDOUT so it can be piped to a file.

To run the task, cd to `/opt/puppet/share/puppet-dashboard`.  Then, run `/opt/puppet/bin/rake RAILS_ENV=production csv:nodes > ~/foo.csv`.
