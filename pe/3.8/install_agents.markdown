...
* **Option 1**
    ...
    If you would like to use the PE-provided repo, you can copy the agent tarball into the `/opt/puppet/staging/pe_repo` directory on your master, and then follow the steps in [Using the PE Agent Package Installation Script](#using-the-pe-agent-package-installation-script) to complete the process.
