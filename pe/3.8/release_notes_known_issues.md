...
### Update NSS Package to Ensure Node Classification Works on Install and Upgrades

The installer/upgrader will skip adding/modifying classification to the node classifier if your Network Security Services (NSS) is not up to date. Ensure you have installed the latest package of NSS so that `curl --tlsv1` works properly during the install/upgrade.
...
### Puppet Dashboard Workers Service Not Restarted After Upgrade

If you have Puppet dashboard workers installed, note that the `pe-puppet-dashboard-workers` service does not automatically re-start when upgrading. You need to restart the service manually after the upgrade. To do so run `puppet resource service pe-puppet-dashboard-workers ensure=running`.
...
### Puppet Enterprise Installer Attempts to Install pe-postgres on External PostgreSQL Databases

When installing PE 3.8.1, if you attempt to install an existing PostgreSQL instance on the same node as the one assigned the PuppetDB component, the install will fail with the following error:

~~~
!! ERROR: Port 5432 appears to be in use. This port is required for the Puppet Enterprise Postgresql Server. Please either move the services for this port to another or install on a system with this port available.
~~~

To workaround the issue, install the external PostgreSQL instance on a server other than the one you've assigned to host the PuppetDB component.
OR

Use the web-based installer to create answer files, and then change the `q_database_install` answer in the PuppetDB answer file to be `n` instead of `y`. Run the answer files on each node (or just the Puppet master in a monolithic install) to complete the installation.
...
A string containing multiple $ signs, such as `"$hostname$hostname"` cannot be escaped properly. For example, if a node has the Facter hostname `centos7`, then `"\$hostname$hostname"` should return `"$hostnamecentos7"`, but instead, it returns`"centos7centos7"`.

### Custom Diffie-Hellman Param File Generation (part of "Log Jam Attack" mitigation)

To help mitigate the "Log Jam Attack," PE 3.8.1 ships with a pre-generated 2048 bit Diffie-Hellman param file. In the case that you don't want to use the PE default DH param file, [Configuring the PE Console to Use a Custom Diffie-Hellman File](./trouble_dh_generate.html) shows how to generate your own.

### Agent-Specified Environment Not Applied Until Second Puppet Run

If you use a fact to trigger the agent-specified environment, it takes two sequential Puppet runs before the agent-specified environment is picked up.

For example, in an **Agent-specified** node group with the **This is an environment group** option selected, if you add a rule that matches nodes with the "test" fact set to true and then you run Puppet, the agent receives the environment that would have been assigned without the "test" fact set to true on the first Puppet run, and then on the second Puppet run it receives the agent-specified environment.

### $ Sign Cannot Be Escaped in Parameter Values

When you use a dollar sign ($) in a parameter value, it is interpolated as a variable. It is not currently possible to escape a dollar sign and pass it in as part of a string.
...
This issue was resolved in PE3.8.3.
...
Note that this issue only affects PE PE 3.8.0.
...
### Classes With Shared Parameter Names Can Cause Schema Errors When Deleted

In rare circumstances, deleting multiple classes can make it impossible to view a group that refers to those classes.

If two classes share a parameter name, and a group declares both classes but only specifies a value for that parameter in one of them, then deleting the classes will make it impossible to retrieve the group.

This bug also occurs if you delete the production environment.

The error message for the bug begins with "An error occurred loading this page: Output of annotate-group* does not match schema: "

This bug will be fixed in a future version of PE.
...
### Workaround to Run the Puppet 4 Language Parser in Non-Production Environments

In PE 3.8.0, a known issue makes it impossible to run Puppet with the Puppet 4 language parser (i.e., the future parser) on non-production environments, as specified in the [documentation](/puppet/3.8/reference/experiments_future.html?_ga=1.7929394.1635187805.1425670182). The following steps makes it possible to work around this issue.

1. In the `[main]` section of `puppet.conf`, set `parser=future`.
2. Restart the pe-puppetserver service.
...
3. For any environments that you don't want to run against the Puppet 4 language parser, in their [`environment.conf` file](/puppet/3.8/reference/config_file_environment.html), set `parser=current`.

   The parser setting in `environment.conf` overrides the settings in `puppet.conf`,  allowing you to control the parser on each environment.

This issue will be fixed in PE 3.8.1.
...
We’ve discovered an issue in which customers on some Ubuntu 14.04 amd64 platforms, such as on a Fusion VM or in Amazon EC2 cannot properly install or upgrade to PE 3.8. Depending on your situation, the current workarounds are available:
...
### AIX and Solaris 10 and 11 Have No Default `symlink` Directory

AIX and Solaris 10 and 11 will not by default have the `symlink` directory in their path. If you use one of these two platforms,  add `/usr/local/bin` to your default path.
